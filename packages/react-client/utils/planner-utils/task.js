export function getNextID(tasks) {
  return tasks.reduce((curentID, task) => {
    if ((task.id !== null || task.id !== undefined) && task.id >= curentID) {
      curentID = task.id + 1
    }
    return curentID
  }, 0)
...
export function getOwnerNames(task, team) {
  ...
  const ownersNames = task.owner.map((ownerKey) => getOwnerName(ownerKey, team))
...
export function getPrimaryOwnerName(task, team) {
  const primaryOwnerKey = Array.isArray(task.owner) ? task.owner[0] : task.owner
